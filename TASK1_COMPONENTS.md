...
- `src/components/Footer.tsx`: Final CTA area and legal/social links.

## Persistent CTA Components
- `src/components/FloatingWhatsApp.tsx`: Floating WhatsApp button.
- `src/components/StickyMobileCTA.tsx`: Mobile sticky footer CTA bar (WhatsApp and Call).

## Tracking, Attribution, and Validation
- `src/hooks/useTracking.ts`: Normalized event payload creation and dispatch.
- `src/hooks/useUtmCapture.ts`: UTM/click-id/session capture from URL and session storage persistence.
